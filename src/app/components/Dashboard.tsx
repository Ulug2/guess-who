...
export function Dashboard() {
  ...
  const [boardFilledCount, setBoardFilledCount] = useState(0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-4">
      <div className="flex justify-between items-start mb-8 pt-2 gap-2">
        <button
          onClick={handleLogout}
          className="bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white px-4 py-2 rounded-lg shadow-lg border-2 border-gray-700 hover:border-[#FFD700]/50 transition-all active:scale-95 flex items-center gap-2 min-h-[44px]"
        >
          <LogOut size={18} />
          <span className="font-semibold">Logout</span>
        </button>
        <div className="bg-[#8B0000] text-white px-4 py-2 rounded-lg shadow-lg border-2 border-[#FFD700] text-right">
          <div className="text-xs opacity-75">Welcome</div>
          <div className="font-bold">{user.username}</div>
          {totalNotifications > 0 && (
            <div className="text-xs mt-1 text-[#FFD700]">
              {incomingChallengesCount > 0 && `${incomingChallengesCount} challenge${incomingChallengesCount > 1 ? "s" : ""}`}
              {incomingChallengesCount > 0 && pendingRequestsCount > 0 && " · "}
              {pendingRequestsCount > 0 && `${pendingRequestsCount} friend request${pendingRequestsCount > 1 ? "s" : ""}`}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-white mb-2">Guess Who?</h1>
          <div className="h-1 w-24 bg-gradient-to-r from-[#8B0000] via-[#FFD700] to-[#8B0000] mx-auto rounded-full"></div>
        </div>

        {!hasMinCharacters && (
          <div className="mb-6 p-4 bg-amber-900/40 border-2 border-amber-500/60 rounded-2xl text-center">
            <p className="text-amber-200 font-semibold">Get started</p>
            <p className="text-amber-100/90 text-sm mt-1">
              Add at least {MIN_CHARACTERS} characters in <strong>Customize Gameboard</strong> to unlock Challenge a Friend.
            </p>
            <p className="text-amber-200/80 text-xs mt-2">
              {boardFilledCount}/{MIN_CHARACTERS} characters added
            </p>
          </div>
        )}
        ...
        <div className="space-y-4">
          <button
            onClick={() => hasMinCharacters && setChallengeModalOpen(true)}
            disabled={!hasMinCharacters}
            className={`w-full py-6 rounded-2xl shadow-xl transition-all duration-300 flex items-center justify-center gap-3 border-2 relative ${
              hasMinCharacters
                ? "bg-gradient-to-r from-[#8B0000] to-[#B22222] hover:from-[#A52A2A] hover:to-[#DC143C] text-white hover:scale-105 hover:shadow-2xl active:scale-95 border-[#FFD700]/30"
                : "bg-gray-700 text-gray-400 border-gray-600 cursor-not-allowed"
            }`}
          >
            <Users size={28} />
            <span className="text-xl font-semibold">Challenge a Friend</span>
            {(pendingRequestsCount > 0 || incomingChallengesCount > 0) && hasMinCharacters && (
          ...
          >
            <UserPlus size={28} />
            <span className="text-xl font-semibold">Add Friends</span>
          ...
          >
            <Grid3x3 size={28} />
            <span className="text-xl font-semibold">Customize Gameboard</span>
          </button>
        </div>
